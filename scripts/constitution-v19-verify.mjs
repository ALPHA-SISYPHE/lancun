...
const FRAC_TOLERANCE = 0.05;
const RATIO_TOLERANCE = 0.03;
const Y_TOLERANCE = 0.025;
const VIEWPORTS = [
...
async function checkViewport(page, width, height, label) {
  ...
  await page.evaluate(async () => {
    ...
    document.querySelector('#ocean-explore')?.scrollIntoView({ block: 'center', behavior: 'instant' });
  });
  await page.waitForTimeout(2500);
  await page.locator('#ocean-explore').screenshot({ path: `constitution-v19-${label}.png` });
...
function okPass(r) {
  ...
  if (r.module !== 'ocean-globe@v19') return false;
...
for (const vp of VIEWPORTS) {
  ...
  report.viewports[vp.label] = { ...r, pass: okPass(r) };
}
report.allPass = Object.values(report.viewports).every((v) => v.pass);

writeFileSync('constitution-v19-verify.json', JSON.stringify(report, null, 2));
