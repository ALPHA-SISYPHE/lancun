...
| 布局法 | 见 `docs/OCEAN_EXPLORE_CONSTITUTION.md`（**v1.9.1 interim**：地球移除；原版气泡 + `ocean-bubbles.js`） |

### 2.2 文档同步要求

- `DESIGN.md` §7 / §9：已声明无例外  
- `docs/DESIGN_SYSTEM.md`：mirror `#ocean-explore` 跟 v4  
- `docs/OCEAN_EXPLORE_CONSTITUTION.md`：**v1.9.1 interim**（地球移除；气泡恢复 pre-v1.9；入口 `ocean-bubbles.js`）  
